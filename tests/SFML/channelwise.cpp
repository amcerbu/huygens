...
inline int process(const float* in, float* out)
{
	...
	{
		out[i] = 0;
		double left_sample = in[CHANS * i];
		double right_sample = in[CHANS * i + 1];
		// double the_sample = metro() * 0.1;
		waveform[waveOrigin + waveSize * flipped] = 
			sf::Vertex(
				sf::Vector2f(
					(1 + gain * left_sample) / 2 * WIDTH,
					(1 + gain * right_sample) / 2 * HEIGHT), 
				sf::Color(LIGHTNESS,LIGHTNESS,LIGHTNESS,ALPHA)
				);
...
int main()
{
	...
	triangle[0].color = sf::Color(DARKNESS, 0, 0, ALPHA); //::Red;
	triangle[1].color = sf::Color(0, 0, DARKNESS, ALPHA); //::Bue;
	triangle[2].color = sf::Color(0, DARKNESS, 0, ALPHA); //::Green;
	triangle[3].color = sf::Color(DARKNESS, DARKNESS, 0, ALPHA); //::Yellow;
