...
sf::Vertex waveforms[2 * waveSize * CHROMATIC];
int waveOrigin = 0;
double gain = 3.0;
double radius = 0.75;
double offset = 0.0;
...
Oscbank<double, CHROMATIC> oscbank;
...
void coefficients(int base)
{
	for (int j = 0; j < CHROMATIC; j++)
	{
		oscbank.freqmod(j, mtof(base) * pow(2, (double)j / CHROMATIC));
	}
}
...
int main()
{
	...
	sf::RenderWindow window(sf::VideoMode(WIDTH, HEIGHT), "Tuner", sf::Style::Titlebar | sf::Style::Close, settings);
	...
	for (int j = 0; j < CHROMATIC; j++)
	{
		xs[j] = radius * sin((2 * PI * j * 1) / CHROMATIC);
		ys[j] = radius * -cos((2 * PI * j * 1) / CHROMATIC);
	}
	oscbank.open();
